...
def rectify(map, beacons):
    ...
    for i, map_anchor in enumerate(map_clone):
        ...
        for center_of_rotation in beacons:
            ...
            rotation = rotation_where_overlap_occurs(map_clone, beacons)
            if rotation:
                beacons = [(point[0]+map_list[i][0],
                            point[1]+map_list[i][1],
                            point[2]+map_list[i][2]) for point in rotation]
...
def rotation_where_overlap_occurs(map, beacons):
    for i, rotation in enumerate(zip(*tuple(rotations.get_rotations(point) for point in beacons))):
        map_set = set(tuple(tuple(point) for point in map))
        intersection = len(map_set.intersection(rotation[:-1]))
